...
class ProcessWindow(QDialog):
    ...
    def set_midi_folder(self):
        directory_list = QFileDialog.getExistingDirectory(
            None, "Select MIDI input Directory")
        if directory_list:
            self.midi_folder_input.setText(directory_list)

    def set_output_file(self):
        self.output_file_input.setText(QFileDialog.getSaveFileName(
            None, 'Create a processed midi file (melodies)', '', '')[0])

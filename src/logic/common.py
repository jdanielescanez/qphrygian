...
def prepare_sequences(melodies, unique_pitches, unique_durations):
    ...
    pitch_to_index = dict((pitch, number)
                          for number, pitch in enumerate(unique_pitches))
    duration_to_index = dict((duration, number)
                             for number, duration in enumerate(unique_durations))

    def to_index(note): return [
        pitch_to_index[note[0]], duration_to_index[note[1]]]
    ...
    for melody in melodies:
        for i in range(0, len(melody) - SEQUENCE_LENGTH):
            sequence_in = [x for note in melody[i:i + SEQUENCE_LENGTH]
                           for x in note]
